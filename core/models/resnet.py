...
class ResNet:
    ...
    def residual_model(data, K, stride, chan_dim, reduce=False, reg=0.0001, bn_eps=2e-5, bn_mom=0.9):
        ...
        conv_2 = Conv2D(int(K*0.25), (3, 3), strides=stride, padding="same", use_bias=False, kernel_regularizer=l2(reg))(act_2)
    ...
    @staticmethod
    def build(width, height, depth, classes, stages, filters, bn_eps=2e-5, bn_mom=0.9, reg=0.0001, dataset="cifar"):
        ...
        for i in range(0, len(stages)):
            ...
            x = ResNet.residual_model(x, filters[i+1], stride, chan_dim, reduce=True)
